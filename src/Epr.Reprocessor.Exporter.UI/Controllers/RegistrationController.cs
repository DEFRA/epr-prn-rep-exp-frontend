	...
	public class RegistrationController : Controller
	{
		...
		public async Task<IActionResult> AddressForNotices()
	   {
		...
		public async Task<IActionResult> ProvideSiteGridReference(ProvideSiteGridReferenceViewModel model, string buttonAction)
		{
			...
			try
			{
				await CreateRegistrationIfNotExistsAsync(session);
			}
			catch (Exception ex)
			{
		...
		public async Task<IActionResult> ProvideGridReferenceOfReprocessingSite(ProvideGridReferenceOfReprocessingSiteViewModel model, string buttonAction)
		{
			...
			var registrationId = await _reprocessorService.Registrations.CreateAsync(new()
			{
				ApplicationTypeId = ApplicationType.Reprocessor,
				OrganisationId = HttpContext.GetUserData().Organisations.FirstOrDefault()!.Id.GetValueOrDefault()
			});
		...
		public async Task<IActionResult> ManualAddressForReprocessingSite(ManualAddressForReprocessingSiteViewModel model, string buttonAction)
		{
			...
			var reprocessingSiteAddress = session.RegistrationApplicationSession.ReprocessingSite!.Address;
            ...
            var request = new UpdateRegistrationRequestDto
            {
                ...
                ReprocessingSiteAddress = new AddressDto
                {
                    ...
                    TownCity = reprocessingSiteAddress.Town,
                    County = reprocessingSiteAddress.County,
                    PostCode = reprocessingSiteAddress.Postcode,
                    NationId = (int)reprocessingSite.Nation,
                    GridReference = reprocessingSite.SiteGridReference
                }
            };

			await _reprocessorService.Registrations.UpdateAsync(request);
		...
		private async Task CreateRegistrationIfNotExistsAsync(ReprocessorRegistrationSession session)
		{
