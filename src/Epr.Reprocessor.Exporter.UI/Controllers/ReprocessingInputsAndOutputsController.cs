		...
		validationService, selectAuthorisationStringLocalizer, requestMapper)
{
	...
	public async Task<IActionResult> PackagingWasteWillReprocess(PackagingWasteWillReprocessViewModel model, string buttonAction)
	{
		...
		{
			if (reprocessingInputsOutputsSession.Materials.Count > 0)
			{
				var materials = reprocessingInputsOutputsSession.Materials.ToList();
		...
		{
			reprocessingInputsOutputsSession.Materials
		...
		{
			if (reprocessingInputsOutputsSession.SelectedMaterials.Count == reprocessingInputsOutputsSession.Materials.Count)
				return Redirect(PagePaths.ApplicationContactName);

			return Redirect(PagePaths.PackagingWasteWillReprocess);
		}
