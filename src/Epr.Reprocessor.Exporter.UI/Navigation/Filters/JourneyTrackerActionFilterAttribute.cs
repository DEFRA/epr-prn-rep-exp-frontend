    ...
    where TSession : class, ISessionData, new()
{
    ...
    public override async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
    {
        ...
        if (session.Journey.IndexOf(GetUrlWithoutPathBase(context.HttpContext.Request)) < 0)
        {
            ...
            {
                session.Journey.Add(GetUrlWithoutPathBase(context.HttpContext.Request));
            }
        }
    ...
    public static string GetUrlWithoutPathBase(HttpRequest request)
    {
        var path = request.Path; // This excludes PathBase
